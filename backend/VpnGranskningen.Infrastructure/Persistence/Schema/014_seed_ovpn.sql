-- Migration 014 – Seed OVPN
-- Jurisdiktion: Sverige (EU, datalagringslag) – risk 3 (hög)
-- Priser i SEK/mån – VERIFIERA mot https://www.ovpn.com/sv/pricing
...
INSERT INTO vpn_provider (
    ...
    '["Netflix", "SVT Play"]',
    8, 100, 30,
    'Svensk aktör som äger sin egen hårdvara och har bevisad no-logs-policy via domstolsbeslut. Stark integritetsprofil.',
    false, '2026-03-12',
    '{
...
SELECT vpn_provider_id, 'main', 'https://www.ovpn.com/sv'
FROM   vpn_provider WHERE slug = 'ovpn'
ON CONFLICT (vpn_provider_id, link_type) DO UPDATE SET url = EXCLUDED.url;

INSERT INTO vpn_provider_links (vpn_provider_id, link_type, url)
SELECT vpn_provider_id, 'price', 'https://www.ovpn.com/sv/pricing'
FROM   vpn_provider WHERE slug = 'ovpn'
ON CONFLICT (vpn_provider_id, link_type) DO UPDATE SET url = EXCLUDED.url;

-- =============================================================================
-- vpn_provider_pricing (SEK/mån) – VERIFIERA PRISERNA
-- OVPN erbjuder månads- och årsplan. Inget 2-årsabonnemang.
-- =============================================================================
