-- Migration 011 – Seed Private Internet Access (PIA)
-- Jurisdiktion: USA (5-Eyes) – risk 3 (hög)
-- Priser i SEK/mån – VERIFIERA mot https://www.privateinternetaccess.com/sv/buy-vpn/
...
INSERT INTO vpn_provider (
    ...
    'Private Internet Access', 'pia', NULL, NULL,
    1,
...
SELECT vpn_provider_id, 'main', 'https://www.privateinternetaccess.com/sv/'
FROM   vpn_provider WHERE slug = 'pia'
ON CONFLICT (vpn_provider_id, link_type) DO UPDATE SET url = EXCLUDED.url;

INSERT INTO vpn_provider_links (vpn_provider_id, link_type, url)
SELECT vpn_provider_id, 'price', 'https://www.privateinternetaccess.com/sv/'
FROM   vpn_provider WHERE slug = 'pia'
ON CONFLICT (vpn_provider_id, link_type) DO UPDATE SET url = EXCLUDED.url;
